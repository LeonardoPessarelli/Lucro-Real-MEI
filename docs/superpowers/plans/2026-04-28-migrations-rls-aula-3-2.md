...
- [ ] **Passo 1: Criar o arquivo de migration com o CLI**
...
- [ ] **Passo 2: Colar o SQL no arquivo gerado**
...
- [ ] **Passo 3: Aplicar a migration no banco remoto via MCP**
...
- [ ] **Passo 4: Verificar que o trigger existe**
...
- [ ] **Passo 5: Verificar as policies novas**
...
- [ ] **Passo 1: Substituir o conteúdo do callback**
...
- [ ] **Passo 3: Rodar os testes existentes**
...
- [ ] **Passo 1: Verificar o trigger via SQL (simula o insert)**
...
- [ ] **Passo 2: Confirmar que as tabelas têm RLS ativo**
...
- [ ] **Passo 3: Confirmar ausência de advisors de segurança**
...
- [ ] `handle_new_user()` usa `security definer` + `set search_path = ''` (impede search_path hijacking)
- [ ] Nenhuma policy usa `raw_user_meta_data` para decisões de autorização (usamos só `auth.uid()` e `auth.role()`)
- [ ] `subscriptions` não tem policies de INSERT/UPDATE/DELETE para `authenticated` ou `anon`
- [ ] `transactions` tem as 4 policies CRUD completas (SELECT, INSERT, UPDATE, DELETE)
- [ ] `profiles` tem as 3 policies necessárias (SELECT, INSERT, UPDATE) — DELETE não é necessário pois o cascade cuida disso
- [ ] `service_role` key nunca aparece em variável `NEXT_PUBLIC_*`
